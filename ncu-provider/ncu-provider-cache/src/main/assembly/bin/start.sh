...
SERVER_NAME=`sed '/server.name/!d;s/.*=//' config/application.properties | tr -d '\r'`
SERVER_PORT=`sed '/dubbo.protocol.port/!d;s/.*=//' config/application.properties | tr -d '\r'`

if [ -z "$SERVER_NAME" ]; then
    echo "ERROR: The $SERVER_NAME is null!"
    echo "PID: $PIDS"
    exit 1
fi
...
nohup java $JAVA_MEM_OPTS -Dserver.home="$DEPLOY_DIR" -jar $DEPLOY_DIR/lib/$SERVER_NAME-1.0-RELEASE.jar $JAVA_OPTS > $STDOUT_FILE 2>&1 &
