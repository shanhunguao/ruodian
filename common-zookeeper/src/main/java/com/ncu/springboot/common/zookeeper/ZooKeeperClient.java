...
import com.ncu.springboot.common.zookeeper.callback.ZooKeeperLockCallback;
import com.ncu.springboot.common.zookeeper.listener.LeaderLatchEventListener;
import com.ncu.springboot.common.zookeeper.listener.NodeDataCacheListener;
import com.ncu.springboot.common.zookeeper.listener.PathChildrenCacheEventListener;
import com.ncu.springboot.common.zookeeper.listener.ZooKeeperClientConnectionStateListener;
import com.ncu.springboot.core.constant.CharsetType;
import com.ncu.springboot.core.jdkextends.ConcurrentHashSet;
import com.ncu.springboot.core.jdkextends.NcuOptional;
import com.ncu.springboot.core.thread.ExecutorServiceManager;
import com.ncu.springboot.core.util.CollectionUtil;
import com.ncu.springboot.core.util.IOUtil;
...
public class ZooKeeperClient {
    ...
    private ExecutorService zkNodeEventNotifierExecutor = ExecutorServiceManager
    ...
    private Map<String, NodeCacheHandler> nodeCacheHandlerMap = new HashMap<>();

    private Lock nodeCacheHandlerMapLock = new ReentrantLock();

    private Map<String, PathChildrenCacheHandler> pathChildrenCacheHandlerMap = new HashMap<>();

    private Lock pathChildrenCacheHandlerMapLock = new ReentrantLock();

    private Map<String, LeaderLatchHandler> leaderLatchHandlerMap = new HashMap<>();
...
//        if (state.isConnected()) {
    	if (state == ConnectionState.CONNECTED) {
            connectionStateListeners.forEach( listener -> listener.notifyConnected() );
        } else {
            connectionStateListeners.forEach( listener -> listener.notifyDisconnected() );
        }
    }
...
//        if (state.isConnected()) {
    ...
    public void create(String path, CreateMode createMode) {
        ...
        if (!exists(path)) {
            try {
                client.create().withMode(createMode).forPath(path, IOUtil.EMPTY_BYTES);
            } catch (Exception e) {
    ...
    public String getStringData(String path, boolean decompress, boolean giveRawIfDecompressFailed) {
        ...
        return null == bytes ? null : new String(bytes, CharsetType.CHARSET_UTF_8);
    }
        ...
        try {
            ...
            if (exists(path)) {
                ...
                if (decompress && 0 < bytes.length) {
                    bytes = ZipUtil.ungzip(bytes, giveRawIfDecompressFailed);
                }
                return bytes;
            } else {// 如果路径不存在则返回null
                return null;
     ...
     */
    public NcuOptional<String> getDataFromCache(String path) {
        // 先确认是否已注册监视
        return NcuOptional.of(nodeCacheHandlerMap.get(path)).getNcuOptional( handler -> handler.getStringData() );
    }
    ...
    public void setData(String path, String data, boolean compress, CreateMode createMode) {
        setData(path, null == data ? IOUtil.EMPTY_BYTES : data.getBytes(CharsetType.CHARSET_UTF_8), compress, createMode);
    }
    ...
    private class NodeCacheHandler implements NodeCacheListener {
        ...
        public String getStringData() {
            byte[] bytes = getData();
            String data = null == bytes ? null : new String(bytes, CharsetType.CHARSET_UTF_8);
            return data;
        }
        ...
        public void nodeChanged() throws Exception {
            if( !CollectionUtil.isEmpty(registeredListeners) ) {
                final byte[] data = getData();
                zkNodeEventNotifierExecutor.submit(() -> {
                    try {
                        registeredListeners.forEach( listener -> listener.nodeChanged(path, data) );
                    } catch (Exception e) {
    ...
    private class PathChildrenCacheHandler implements PathChildrenCacheListener {
        ...
        public void stopWatch() {
            if (started) {
                IOUtil.close(pathChildrenCache);
                started = false;
            }
        }
        ...
        private void addRecursionWatchListener(PathChildrenCacheEventListener listener) {
            if (recursionWatchListeners.add(listener)) {
                // 遍历当前的子路径
                childHandlers.entrySet().forEach( entry -> {
                    // 查看当前是否已获取handler
        ...
        private void removeChildHandler(String childPath) {
        	NcuOptional.of(childHandlers.remove(childPath)).ifPresent(childHandler -> childHandler.clearListener() );
        }
        ...
        private void notifyAdded(final String path, final byte[] data, final Stat stat) {
            if( !CollectionUtil.isEmpty(registeredListeners) ) {
                zkNodeEventNotifierExecutor.submit(() -> {
                    try {
                        registeredListeners.forEach(listener -> listener.childPathAdded(path, data, stat) );
                    } catch (Exception e) {
        ...
        private void notifyUpdated(final String path, final byte[] data, final Stat stat) {
            if( !CollectionUtil.isEmpty(registeredListeners) ) {
                zkNodeEventNotifierExecutor.submit(() -> {
                    try {
                        registeredListeners.forEach( listener -> listener.childPathUpdated(path, data, stat) );
                    } catch (Exception e) {
        ...
        private void notifyRemoved(final String path, final Stat stat) {
            if( !CollectionUtil.isEmpty(registeredListeners) ) {
                zkNodeEventNotifierExecutor.submit(() -> {
                    try {
                        registeredListeners.forEach(listener -> listener.childPathRemoved(path, stat) );
                    } catch (Exception e) {
        ...
        private void showCurrentData(PathChildrenCacheEventListener listener) {
            pathChildrenCache.getCurrentData().forEach( dataObj -> {
                if (null != dataObj) {
                    listener.childPathAdded(dataObj.getPath(), dataObj.getData(), dataObj.getStat());
                }
            });
        }
        ...
        public void clearListener() {
            ...
            recursionWatchListeners.forEach( listener -> removeRecursionWatchListener(listener));
        }
    ...
    public void unregisterPathChildrenWatch(String path) {
        ...
        PathChildrenCacheHandler pathChildrenCacheHandler = pathChildrenCacheHandlerMap.get(path);
        if (null != pathChildrenCacheHandler) {
            pathChildrenCacheHandler.stopWatch();
    ...
    private class LeaderLatchHandler implements LeaderLatchListener {
        ...
        public void isLeader() {
            if( !CollectionUtil.isEmpty(registeredListeners) ) {
                zkNodeEventNotifierExecutor.submit(() -> {
                    try {
                        registeredListeners.forEach(listener -> listener.isLeader());
                    } catch (Exception e) {
        ...
        public void notLeader() {
            if( !CollectionUtil.isEmpty(registeredListeners) ) {
                zkNodeEventNotifierExecutor.submit(() -> {
                    try {
                        registeredListeners.forEach(listener -> listener.notLeader());
                    } catch (Exception e) {
                        LOG.error("notify isLeader failed", e);
                    }
                });
    ...
    public void closeLeaderLatch(String path) {
        ...
        NcuOptional.of(leaderLatchHandlerMap.get(path)).ifPresent( handler -> {
     ...
     * @param runnable 任务。
     * @throws EOFException         等待过程中竞争器被关闭了则会抛出该异常。
